...
class BackupsController extends ClientApiController
{
    ...
    public function destroy(Request $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        $this->assertDestructiveConfirmation($request);
    ...
    public function restore(RestoreBackupRequest $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        $log = Activity::event('server:backup.restore')
            ->subject($backup)
            ->property(['name' => $backup->name, 'truncate' => $request->boolean('truncate_directory')]);
        ...
        $log->transaction(function () use ($backup, $server, $request) {
            $this->backupCoordinator->restore(
                ...
                $backup,
                $request->user(),
                $request->boolean('truncate_directory'),
            );
        });
    ...
    public function deleteAll(Request $request, Server $server): JsonResponse
    {
        ...
        $deleted = $this->backupCoordinator->deleteAll($server);

        Activity::event('backup:delete_all')
            ->subject($server)
            ->property(['backup_count' => $deleted])
            ->log();
    ...
    public function bulkDelete(Request $request, Server $server): JsonResponse
    {
        ...
        foreach ($backups as $backup) {
            try {
                ...
                \Log::error("Failed to delete backup {$backup->uuid}", [
                    'error' => $e->getMessage(),
                ]);
            }
        }
    ...
    /**
     * Require password/2FA for destructive ops on web sessions (not API keys).
     */
    private function assertDestructiveConfirmation(Request $request): void
    {
